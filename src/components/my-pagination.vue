<template>
	<div>
		<div class="g-block">
			<template v-if="paginationObj.saveBtn">
				<el-button class="g-top-btn" size="mini" type="primary" @click="saveBtnData()">保存</el-button>
			</template>
			<template v-if="paginationObj.applyBtn">
				<el-button class="g-top-btn" size="mini" type="primary" @click="saveBtnData()">申请</el-button>
			</template>
		  <el-pagination
		  	class="s-page-style"
...
export default {
	...
	methods: {
		changeCur (val) {  //分页按钮点击事件
			let self = this;
			self.$emit('changeCur', val)
		},
		saveBtnData () {
			let self = this;
			self.$emit('saveBtnData', '成功');
		}
	}
}
</script>
