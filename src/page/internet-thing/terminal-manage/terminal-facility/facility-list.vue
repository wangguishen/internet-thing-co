	...
	<div>
		...
		<div class="g-main">
		    ...
		    <el-table-column
		      ...
		      label="所属公司">
		      <template slot-scope="scope">
		      	<span :title="scope.row.merName || compName">{{scope.row.merName || compName}}</span>
		      </template>
		    </el-table-column>
		    <el-table-column
		      align="center"
		      ...
		      <template slot-scope="scope">
		      	...
		      	<template v-else>
		      		<span class="iconfont" style="color:#00aaee;font-size: 16px;">&#xe67e;</span>
		      	</template>
		      </template>
		    </el-table-column>
		  </el-table>
...
export default {
	...
	mounted () {
		...
		self.compName = getLocalStorage('USER_INFO', 'json').compName || ''
		let data = {
			"commandId": 0x0541,
			"reqNode": "WEB",
			"body": {
	...
	methods: {
        ...
        return
			...
			if (companyData.status == 200 && companyData.data.retCode == 0) {
				...
				self.companyList = obj.deviceList;
			} else {
				self.$message.error(companyData.data.errMsg)
			}
		},
